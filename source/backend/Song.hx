...
public static function convert(songJson:Dynamic):Void
{
	...
	for (section in songJson.notes)
	{
		...
		var beats:Null<Float> =
			Std.parseFloat(Std.string(section.sectionBeats));
		...
		for (rawNote in section.sectionNotes)
		{
			...
			if (note.length > 2)
			{
				...
				note[2] = (sus == null || Math.isNaN(sus) || sus < 0)
					? 0
					: sus;
			}
			...
			note[1] =
				(lane % 4) + (gottaHitNote ? 0 : 4);

			if (
				note.length > 3
				&& note[3] != null
				&& !Std.isOfType(note[3], String)
			)
			{
				...
				if (
					idx >= 0
					&& idx < Note.defaultNoteTypes.length
				)
					note[3] =
						Note.defaultNoteTypes[idx];
				else
					note[3] = "";
			}
...
):SwagSong
{
	...
	var formattedFolder =
		Paths.formatToSongPath(folder);
	var formattedSong =
		Paths.formatToSongPath(jsonInput);

	_lastPath =
		Paths.json('$formattedFolder/$formattedSong');
...
):SwagSong
{
	...
	{
		var subSong =
			Reflect.field(songJson, 'song');

		if (
			subSong != null
			&& Type.typeof(subSong) == TObject
		)
			songJson = subSong;
	}
	...
	{
		trace(
			'converting chart $nameForError with format $fmt...'
		);
