 ...
 *
 * If you want to make a custom note type, search for: "function set_noteType"
**/
class Note extends FlxSprite
{
    // This list appears in the Chart Editor
    public static final defaultNoteTypes:Array<String> = [
        ...
        'GF Sing',
        'No Animation',
        'Death Note'   // ← Your new custom note type
    ];
    ...
    private function set_noteType(value:String):String {
        ...
        if (noteData > -1 && noteType != value) {
            defaultRGB(); // Reset to default colors first
            ...
            switch (value) {
                ...
                case 'Hey!':
                    // Hey! note - can be extended if needed

                case 'Death Note':
                    // === CUSTOM DEATH NOTE ===
                    hitHealth = 0.0;           // No health gain on hit
                    missHealth = 2.0;          // Heavy damage / instant death on miss
                    hitCausesMiss = true;      // Even hitting it counts as a miss
                    lowPriority = true;

                    // Deadly red/black colors
                    rgbShader.r = 0xFF220000;
                    rgbShader.g = 0xFFFF0000;
                    rgbShader.b = 0xFF880000;

                    // Custom splash (create noteSplashes/noteSplashes-death.png if you want)
                    noteSplashData.texture = 'noteSplashes/noteSplashes-death';
                    noteSplashData.r = 0xFFFF0000;
                    noteSplashData.g = 0xFF440000;
                    noteSplashData.b = 0xFF000000;

                    animSuffix = '-death';     // Optional: use -death animation suffix on character
                    // reloadNote('DeathNote_assets'); // Uncomment if you made a custom texture
            }

            // Apply any extra config from NoteTypesConfig (useful for complex note types)
            if (value != null && value.length > 1)
                NoteTypesConfig.applyNoteTypeData(this, value);

            // Precache custom hitsound
    ...
    public function new(strumTime:Float, noteData:Int, ?prevNote:Note, ?sustainNote:Bool = false, ?inEditor:Bool = false, ?createdFrom:Dynamic = null) {
        ...
        if (noteData > -1) {
            ...
            if (!ClientPrefs.data.noteShaders || (PlayState.SONG != null && PlayState.SONG.disableNoteRGB))
