...
class RGBShaderReference
{
	...
	public function new(owner:FlxSprite, ref:RGBPalette)
	{
		...
		// Respect the noteShaders option right away
		_owner.shader = ClientPrefs.data.noteShaders ? ref.shader : null;
		...
		enabled = ClientPrefs.data.noteShaders;
	}
	...
	{
		var canUse:Bool = ClientPrefs.data.noteShaders && value;
		_owner.shader = canUse ? parent.shader : null;
	...
	private function cloneOriginal()
	{
		if(allowNew)
		{
			...
			_owner.shader = ClientPrefs.data.noteShaders ? parent.shader : null;
		}
	}
}
